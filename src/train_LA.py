...
DATA_PATH = os.path.join(BASE_DIR, 'data', 'avocado_processed.csv')
...
def train_model():
    ...
    # features
    # descomponer fecha en semana/mes/año para el modelo
    df['month'] = df['Date'].dt.month
    df['year'] = df['Date'].dt.year
    df['week'] = df['Date'].dt.isocalendar().week.astype(int)
    df['type_organic'] = df['type'].apply(lambda x: 1 if x == 'organic' else 0).astype(int) #convertir categorias type a numeros, creando nueva columna numerica type_organic 
